...
def synth_to_way(path, freq=440.0, duration=0.6, volume=0.6, wave_fn=None, sr=SR):
    n=int(duration*sr)
...
def generate_random_bank(out_dir, count, seed=None):
    ...
    paths=[]
    base_freqs = [130.81, 146.83, 164.81, 174.61, 196.00, 220.00, 246.94, 261.63, 293.66, 329.63, 349.23, 392.00, 440.00, 493.88]
    for i in range(count):
        ...
        synth_to_way(path, freq=freq, duration=dur, volume=vol)
        paths.append(path)
    return paths